...
(Year,Quarter,Month,DayofMonth,DayOfWeek,FlightDate,UniqueCarrier,
	...
	Distance,DistanceGroup,CarrierDelay,WeatherDelay,NASDelay,
	SecurityDelay,LateAircraftDelay);
--remove the records in Flights1 that don't have Origin "ORD" 
...
--remove the records in Flights2 that don't have Dest "JFK" and 
--are not cancelled or diverted
...
NeededInfo1 = FOREACH filterOrigin GENERATE Year, Month, 
		FlightDate, Dest, ArrTime, ArrDelayMinutes;
NeededInfo2 = FOREACH filterDest GENERATE Year, Month, 
		FlightDate, Origin, DepTime, ArrDelayMinutes;
...
--filter out the tuples whose flightDate are not 
--in the required range(June 2007-May 2008)
filter_range = FILTER filter_twoleg BY (($0 eq '2007' and (not ($1 < 6))) 
										or ($0 eq '2008' and (not ($1 > 5))))
				and (($6 eq '2007' and (not ($7 < 6))) 
					or ($6 eq '2008' and (not ($7 > 5))));
--calcute the delay time for each two-leg flight
calcu_delays = FOREACH filter_range GENERATE ($5 + $11) AS delayMinutes;
